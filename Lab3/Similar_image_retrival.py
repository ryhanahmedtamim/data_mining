...
def load_training_feature():
    ...
    global training_feature_book_name
    training_feature_book_name = name
...
def load_test_feature():
    ...
    global test_feature_book_name
    test_feature_book_name = name
...
def result_using_random_forrest():
    sc = StandardScaler()
    train_data =pd.read_excel(training_feature_book_name)
    x_train = train_data.iloc[:, 1:train_data.shape[1]].values
    x_train = sc.fit_transform(x_train)
    y_train = train_data.iloc[:, 0].values

    test_data = pd.read_excel(test_feature_book_name)
    x_test = test_data.iloc[:, 1:train_data.shape[1]].values
    ...
    for i in range(len(y_pred)):
        result_cell = result_sheet.cell(row, 1)
        result_cell.value = y_test[i]
        result_cell = result_sheet.cell(row, 2)
        result_cell.value = str(y_pred[i])
...
test_feature_book_name = StringVar()
...
extract_feature_button_SIFT = Button(frame1, text="Extract SIFT Feature",
                                     height=2, command=lambda: feature_extraction_sift(training_folder_path, 'output_of_train_data_sift2.xlsx', workbook))
...
extract_feature_button_SURF = Button(frame1, text="Extract SURF Feature",
                                     height=2, command=lambda: feature_extraction_surf(training_folder_path, 'output_of_train_data_surf2.xlsx', workbook))
...
test_extract_feature_button_SIFT = Button(frame3, text="Extract SIFT Feature",
                                          height=2, command=lambda: feature_extraction_sift(test_folder_path, 'output_of_test_data_sift2.xlsx', workbook))
...
test_extract_feature_button_SURF = Button(frame3, text="Extract SURF Feature",
                                          height=2, command=lambda: feature_extraction_surf(test_folder_path, 'output_of_test_data_surf2.xlsx', workbook))
...
random_forest_result_button = Button(frame4, text = "Random Forest", height=2, command=result_using_random_forrest)
